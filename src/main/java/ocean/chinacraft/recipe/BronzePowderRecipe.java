...
public class BronzePowderRecipe extends SpecialCraftingRecipe {
    int hammerSlot;

    public BronzePowderRecipe(Identifier id) {
        super(id);
    }
    ...
    public boolean matches(CraftingInventory inventory, World world) {
        ...
        for (int i =0;i<inventory.size();i++){
            ...
            if (!stack.isEmpty()){
                ...
                }else if (TagHelper.Items.checkByIStack(stack,TagsRegister.Items.C_BRONZE_INGOTS)){
                    stackArrayList.add(stack);
                }
            }
    ...
    public ItemStack craft(CraftingInventory inventory) {
        ...
        for (int i =0;i<inventory.size();i++){
            ...
            if (!stack.isEmpty()){
                count++;
                if (TagHelper.Items.checkByIStack(stack,TagsRegister.Items.C_BRONZE_INGOTS)){
                    hasIngot = true;
        ...
        if (hasHammer && hasIngot && count == 2){
            return ItemRegister.BRONZE_POWDER.getDefaultStack();
        }
    ...
    public RecipeSerializer<?> getSerializer() {
        return RecipeRegister.BRONZE_DUST_RECIPE_RECIPE_SERIALIZER;
    }
}
